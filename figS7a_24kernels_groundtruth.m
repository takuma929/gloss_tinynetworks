...
disp('Generating figure S7a...')
%% Load, visualize and save all kernel images from the single-kernel model
cnt = 0;

% create a folder
if ~exist(fullfile(pwd, 'figs', 'onelayer_kernel_groundtruth'),'file')
    mkdir(fullfile(pwd, 'figs', 'onelayer_kernel_groundtruth'))
end
    
for type = {'lighting', 'shape'}
    for N = 1:12
        kernel_dir = fullfile('data', 'networks', 'onelayer_models', ['groundtruth_kernelN1_', type{1}, num2str(N)]);
        ...
        % Save figure
        saveas(gca, fullfile('figs', 'onelayer_kernel_groundtruth', ...
...
for type = {'lighting', 'shape'}
    for N = 1:12
        ...
        imgPath = fullfile('figs', 'onelayer_kernel_groundtruth', ...
            ['kernel', num2str(N), '_', type{1}, '.png']);
...
figure;
imshow(I_out);
imwrite(I_out, fullfile('figs', 'figS7a(onelayer_24kernels_groundtruth).png'));
